...
public class TesseractLanguage extends GuiScreen {
    ...
    private static final int BTN_W = 310, BTN_H = 20, GAP = 4, VISIBLE = 14;
    private List<Language> languages;
    private int scrollOffset = 0;
    private Language selected;
    private final float[] rowHover = new float[VISIBLE + 2];

    public TesseractLanguage(GuiScreen parent, GameSettings settings, LanguageManager langManager) {
        this.parent = parent;
    ...
    @Override
    public void initGui() {
        languages = new ArrayList<>(langManager.getLanguages());
        selected = langManager.getCurrentLanguage();
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        int lx = leftX(), ly = listY();
        drawRect(lx - 2, ly - 2, lx + BTN_W + 2, ly + VISIBLE * (BTN_H + GAP) + 2, 0x33000000);

        for (int i = 0; i < VISIBLE && (i + scrollOffset) < languages.size(); i++) {
            int idx = i + scrollOffset;
            Language lang = languages.get(idx);
            int ry = ly + i * (BTN_H + GAP);
            boolean isSel = lang.equals(selected);
            boolean hov = mouseX >= lx && mouseX <= lx + BTN_W && mouseY >= ry && mouseY <= ry + BTN_H;
            rowHover[i] = hov ? Math.min(1f, rowHover[i] + 0.1f) : Math.max(0f, rowHover[i] - 0.07f);
            float ha = rowHover[i];

            int bg = isSel ? (0x55378ADD | ((int)(0x55 + ha * 0x33) << 24)) : ((int)(0x11 + ha * 0x22) << 24) | 0x1A4A8A;
            drawRect(lx, ry, lx + BTN_W, ry + BTN_H, bg);
            drawBorder(lx, ry, lx + BTN_W, ry + BTN_H, isSel ? 0xAA85B7EB : ((int)(0x33 + ha * 0x33) << 24) | 0x85B7EB);

            String label = lang.toString();
            int lw = mc.fontRendererObj.getStringWidth(label);
            mc.fontRendererObj.drawString(label, lx + BTN_W / 2 - lw / 2, ry + BTN_H / 2 - 3,
                    isSel ? 0xFFFFFFFF : ((int)(0x88 + ha * 0x44) << 24) | 0xC8D8F0);
        }

        // scrollbar
        if (languages != null && languages.size() > VISIBLE) {
            int sbH = VISIBLE * (BTN_H + GAP);
            int thumbH = Math.max(20, sbH * VISIBLE / languages.size());
            int thumbY = ly + (sbH - thumbH) * scrollOffset / (languages.size() - VISIBLE);
            drawRect(lx + BTN_W + 4, ly, lx + BTN_W + 8, ly + sbH, 0x44378ADD);
            drawRect(lx + BTN_W + 4, thumbY, lx + BTN_W + 8, thumbY + thumbH, 0xAA85B7EB);
        }

        int doneY = listY() + VISIBLE * (BTN_H + GAP) + GAP * 2;
        drawStyledBtn(lx, doneY, "Done", VISIBLE, mouseX, mouseY);
    ...
    private void drawStyledBtn(int bx, int by, String label, int idx, int mouseX, int mouseY) {
        ...
        rowHover[idx] = hov ? Math.min(1f, rowHover[idx] + 0.1f) : Math.max(0f, rowHover[idx] - 0.07f);
        float ha = rowHover[idx];
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if (mouseButton != 0 || languages == null) return;
        int lx = leftX(), ly = listY();
        for (int i = 0; i < VISIBLE && (i + scrollOffset) < languages.size(); i++) {
            int ry = ly + i * (BTN_H + GAP);
            if (mouseX >= lx && mouseX <= lx + BTN_W && mouseY >= ry && mouseY <= ry + BTN_H) {
                selected = languages.get(i + scrollOffset);
                langManager.setCurrentLanguage(selected);
                settings.language = selected.getLanguageCode();
                mc.refreshResources();
                mc.fontRendererObj.setUnicodeFlag(langManager.isCurrentLocaleUnicode());
                mc.fontRendererObj.setBidiFlag(langManager.isCurrentLanguageBidirectional());
                settings.saveOptions();
                return;
            }
        }
        int doneY = listY() + VISIBLE * (BTN_H + GAP) + GAP * 2;
        if (mouseX >= lx && mouseX <= lx + BTN_W && mouseY >= doneY && mouseY <= doneY + BTN_H) mc.displayGuiScreen(parent);
    }
    ...
    public void handleMouseInput() throws IOException {
        ...
        if (scroll != 0) scrollOffset = Math.max(0, Math.min(languages.size() - VISIBLE, scrollOffset - (scroll / 120)));
    }
