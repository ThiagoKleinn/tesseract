...
public class TesseractSnooper extends GuiScreen {
    ...
    private final float[] btnHover = new float[3];

    private static final String[] INFO_LINES = {
            "The Snooper collects anonymous data about your",
            "game to help Mojang improve Minecraft.",
            "No personal information is ever collected.",
            "",
            "Data includes: game version, OS, Java version,",
            "memory, world size and play time."
    };

    public TesseractSnooper(GuiScreen parent, GameSettings settings) {
        this.parent = parent;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawHeader("SNOOPER SETTINGS");

        int lx = leftX();

        // info box
        int boxY = rowY(0);
        int boxH = INFO_LINES.length * 10 + 12;
        drawRect(lx, boxY, lx + BTN_W, boxY + boxH, 0x33000000);
        drawBorder(lx, boxY, lx + BTN_W, boxY + boxH, 0x44378ADD);
        for (int i = 0; i < INFO_LINES.length; i++) {
            if (!INFO_LINES[i].isEmpty()) {
                int lw = mc.fontRendererObj.getStringWidth(INFO_LINES[i]);
                mc.fontRendererObj.drawString(INFO_LINES[i], lx + BTN_W / 2 - lw / 2, boxY + 6 + i * 10, 0x88C8D8F0);
            }
        }

        int toggleY = boxY + boxH + GAP * 2;
        String snoopLabel = "Send anonymous data: " + (settings.snooperEnabled ? "ON" : "OFF");
        drawStyledButton(lx, toggleY, snoopLabel, 0, mouseX, mouseY);

        drawStyledButton(lx, toggleY + (BTN_H + GAP) * 2, "Done", 1, mouseX, mouseY);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        int lx = leftX();
        int boxH = INFO_LINES.length * 10 + 12;
        int toggleY = rowY(0) + boxH + GAP * 2;

        if (hit(mouseX, mouseY, lx, toggleY)) {
            settings.snooperEnabled = !settings.snooperEnabled;
            settings.saveOptions(); return;
        }
        if (hit(mouseX, mouseY, lx, toggleY + (BTN_H + GAP) * 2)) mc.displayGuiScreen(parent);
    }
