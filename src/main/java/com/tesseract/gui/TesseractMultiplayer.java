...
public class TesseractMultiplayer extends GuiMultiplayer {
    ...
    private static final String[] BTN_LABELS = {
            "Join Server", "Direct Connect", "Add Server", "Edit", "Delete", "Refresh", "Cancel"
    };
    private static final int[] BTN_IDS = { 1, 4, 3, 7, 2, 8, 0 };
    ...
    // linha 51 — renomear o método
    private GuiSlot getRealServerList() {
        try {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        int row2Y = height - 28;
        int[][] layout = {
                { width/2 - 154, row1Y, 100, 0 },
                { width/2 - 50,  row1Y, 100, 1 },
                { width/2 + 54,  row1Y, 100, 2 },
                { width/2 - 154, row2Y, 70,  3 },
                { width/2 - 74,  row2Y, 70,  4 },
                { width/2 + 4,   row2Y, 70,  5 },
                { width/2 + 80,  row2Y, 75,  6 },
        };
