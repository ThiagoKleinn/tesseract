...
public class TesseractChatOptions extends GuiScreen {
    ...
    // indices: 0=visibility,1=colors,2=links,3=prompt,4-8=sliders,9=Done
    private final float[] btnHover = new float[10];

    private float opacity, scale, chatWidth, focusHeight, unfocusedHeight;
    private int dragging = -1;

    public TesseractChatOptions(GuiScreen parent, GameSettings settings) {
        this.parent   = parent;
        ...
        focusHeight     = settings.chatHeightFocused;
        unfocusedHeight = settings.chatHeightUnfocused;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawStyledButton(lx, rowY(0), "Chat: " + visibilityName(settings.chatVisibility), 0, mouseX, mouseY);
        drawStyledButton(lx, rowY(1), "Colors: "          + onOff(settings.chatColours),    1, mouseX, mouseY);
        drawStyledButton(lx, rowY(2), "Web Links: "       + onOff(settings.chatLinks),       2, mouseX, mouseY);
        drawStyledButton(lx, rowY(3), "Prompt on Links: " + onOff(settings.chatLinksPrompt), 3, mouseX, mouseY);

        drawSlider(lx, rowY(4), "Opacity",          opacity,         4, mouseX, mouseY);
        drawSlider(lx, rowY(5), "Scale",            scale,           5, mouseX, mouseY);
        drawSlider(lx, rowY(6), "Width",            chatWidth,       6, mouseX, mouseY);
        drawSlider(lx, rowY(7), "Focused Height",   focusHeight,     7, mouseX, mouseY);
        drawSlider(lx, rowY(8), "Unfocused Height", unfocusedHeight, 8, mouseX, mouseY);

        drawStyledButton(lx, rowY(10), "Done", 9, mouseX, mouseY);
    ...
    private void drawSlider(int bx, int by, String label, float value, int idx, int mouseX, int mouseY) {
        ...
        drawRect(bx + 1, by + 1, bx + 1 + fillW, by + BTN_H - 1,
                ((int)(0x55 + ha * 0x33) << 24) | 0x378ADD);

        String text = label + ": " + (int)(value * 100) + "%";
        int lw = mc.fontRendererObj.getStringWidth(text);
        mc.fontRendererObj.drawString(text, bx + BTN_W / 2 - lw / 2, by + BTN_H / 2 - 3,
                ((int)(0xAA + ha * 0x55) << 24) | 0xC8D8F0);
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (hit(mouseX, mouseY, lx, rowY(0))) { settings.chatVisibility = nextVisibility(settings.chatVisibility); settings.saveOptions(); return; }
        if (hit(mouseX, mouseY, lx, rowY(1))) { settings.chatColours    = !settings.chatColours;    settings.saveOptions(); return; }
        if (hit(mouseX, mouseY, lx, rowY(2))) { settings.chatLinks      = !settings.chatLinks;      settings.saveOptions(); return; }
        if (hit(mouseX, mouseY, lx, rowY(3))) { settings.chatLinksPrompt = !settings.chatLinksPrompt; settings.saveOptions(); return; }
        ...
        if (hit(mouseX, mouseY, lx, rowY(10))) { applySliders(); mc.displayGuiScreen(parent); }
    }
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        if (dragging >= 0) { applySliders(); dragging = -1; }
    }

    private void updateSlider(int idx, int mouseX, int lx) {
        float v = Math.max(0f, Math.min(1f, (float)(mouseX - lx) / BTN_W));
        switch (idx) {
            case 0: opacity         = v; break;
            case 1: scale           = v; break;
            case 2: chatWidth       = v; break;
            case 3: focusHeight     = v; break;
            case 4: unfocusedHeight = v; break;
        }
    }
    ...
    private void applySliders() {
        ...
        settings.chatWidth           = chatWidth;
        settings.chatHeightFocused   = focusHeight;
        settings.chatHeightUnfocused = unfocusedHeight;
        settings.saveOptions();
    }

    @Override
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        if (keyCode == org.lwjgl.input.Keyboard.KEY_ESCAPE) { applySliders(); mc.displayGuiScreen(parent); }
    }
    ...
    private String visibilityName(EnumChatVisibility v) {
        switch (v) {
            case FULL:    return "Shown";
            case SYSTEM:  return "Commands Only";
            case HIDDEN:  return "Hidden";
            default:      return "Shown";
        }
    }

    private EnumChatVisibility nextVisibility(EnumChatVisibility v) {
        switch (v) {
            case FULL:   return EnumChatVisibility.SYSTEM;
            case SYSTEM: return EnumChatVisibility.HIDDEN;
            default:     return EnumChatVisibility.FULL;
        }
    }
